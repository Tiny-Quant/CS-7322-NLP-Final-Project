...
We hoped that these auxiliary features allowed the model to fine-tune the attention between the sentence, the specific targeted words, and key terms in the two possible definitions. 
...
We trained each model for 100 epochs using the AdamW optimizer with a learning rate of $2 \times 10^{-5}$ and an input batch size of 128. Each model trained in less than 15 minutes of M3.   
